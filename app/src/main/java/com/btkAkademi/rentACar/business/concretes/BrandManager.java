...
public class BrandManager implements BrandService{
	...
	public DataResult<List<BrandListDto>> getAll() {
		...
		List<BrandListDto> response = (List<BrandListDto>) brandList.stream()
				          .map(brand->modelMapperService.forDto().map(brand, BrandListDto.class))
	...
	private Result checkIfBrandIdExists(int id) {
		...
		if(this.brandDao.findById(id).isEmpty()) {
			return new ErrorResult(Messages.brandIsNotUptade);
		}
		return new SuccessResult();
	}
